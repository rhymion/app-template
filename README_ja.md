...
アプリケーションのスキーマは `prj/code_generator/schema/` 配下の YAML ファイルで定義されています。
スキーマを変更した後はコードジェネレーターを実行してアプリケーションファイルを再生成する必要があります。

### 方法 A: 手動で編集する

1. `prj/code_generator/schema/` 配下の YAML ファイルを編集します
2. スキーマのリファレンスドキュメントを参照してください:
   - `app-generator/docs/knowledge/schema-yaml-configuration.md`
   - app-generator リポジトリのオンラインドキュメント
3. コード生成を実行します: `npm run generate-code`
4. 生成されたコードのビルドを確認します: `npm run build`

### 方法 B: AI に依頼する（Claude Code / Codex）

Claude Code または Codex を使ってスキーマを設計・更新できます:
...
**Codex:**
`generate-schema` タスクを選択して、やりたいことを説明してください。

AI が行う処理:
- `prj/code_generator/schema/` にスキーマ YAML を作成・更新します
- `npm run generate-code` でアプリケーションファイルを再生成します
- ビルドが通ることを確認します
